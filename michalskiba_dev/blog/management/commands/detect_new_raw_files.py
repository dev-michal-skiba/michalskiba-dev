...
class Command(BaseCommand):
    help = f"Detect new blog post raw files in {settings.BLOG_POSTS_RAW_URL}"

    def handle(self, *args: list[Any], **options: dict[str, Any]) -> None:
        logger.info("Detecting new blog post raw files")
        for root, sub_dirs, file_names in os.walk(settings.BLOG_POSTS_RAW_URL):
            for file_name in file_names:
                file_path = f"{root}/{file_name}"
                relative_file_path = os.path.relpath(file_path, settings.BLOG_POSTS_RAW_URL)
